...
static struct semaphore *mutex_lock_a;
static struct semaphore *mutex_lock_b;
...
int twolocks (int data1, char ** data2)
{
        ...
        KASSERT(finished != 0); /* KASSERT panics if the condition is
                                   ...
                                   better. */
        mutex_lock_a = sem_create("mutex_lock_a", 1);
        mutex_lock_b = sem_create("mutex_lock_b", 1);
        KASSERT(mutex_lock_a != 0 && mutex_lock_b != 0);
        ...
        P(finished);
        P(finished);
        sem_destroy(mutex_lock_a);
        sem_destroy(mutex_lock_b);
